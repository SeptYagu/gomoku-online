...
async function main(): Promise<void> {
  ...
  try {
    console.log(`Game records smoke: ${baseUrl}`);
    ...
    const created = requireRoomOk(
      await emitAck<RoomAck>(host, "room:create", {
        playerId: `record-host-${suffix}`,
        playerName: `Record Host ${suffix}`
      }),
      "room:create"
    ).snapshot;
    ...
    requireRoomOk(
      await emitAck<RoomAck>(guest, "room:join", {
        playerId: `record-guest-${suffix}`,
        playerName: `Record Guest ${suffix}`,
        roomCode
      }),
      "room:join"
    );
    ...
    assert(guestRecord.record.recordStatus === "verified", "second submission should verify the record");
    assert(guestRecord.record.submissions.length === 2, "verified record should have two submissions");
    console.log(`PASS second submit verified - ${finished.gameId}`);

    const duplicate = requireRecordOk(
      await emitAck<GameRecordAck>(host, "game-record:submit", createGameRecordPayload(finished)),
      "duplicate game-record:submit"
    );

    assert(duplicate.duplicate, "duplicate submission should be reported as duplicate");
    assert(duplicate.record.submissions.length === 2, "duplicate submission should not add a third submission");
    console.log(`PASS duplicate submit deduped - ${finished.gameId}`);
  } finally {
    host.disconnect();
    guest.disconnect();
  }
