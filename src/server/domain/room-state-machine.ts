...
import type { AuthoritativeGameRecord, GameRecordFinishReason } from "../game-records";
...
export type RoomPresenceLocation = {
  code: string;
  role: RoomParticipantRole;
  status: RoomStatus;
  visibility: RoomVisibility;
...
export type RoomLifecycleLimits = {
...
export type RoomStateMachineOptions = {
  ...
  onGameFinished?: (game: AuthoritativeGameRecord) => void;
};
...
export class RoomStateMachine {
  ...
  private readonly getOnlineUserCount: () => number;
  private readonly onGameFinished?: (game: AuthoritativeGameRecord) => void;

  constructor(options: RoomStateMachineOptions = {}) {
    ...
    this.getOnlineUserCount = options.getOnlineUserCount ?? (() => 0);
  ...
  sendPublicChat(input: CreateRoomInput & { text: string }): RoomResult<PublicChatSnapshot> {
    ...
    const now = this.now();
    this.pruneTransientChatRateLimits(now);
  ...
  buildRoomPresenceIndex(): Map<string, RoomPresenceLocation> {
  ...
  private pruneTransientChatRateLimits(now: number): void {
...
export function success<T>(value: T): RoomResult<T> {
  return { ok: true, value };
}

export function failure(code: RoomErrorCode, message: string): RoomResult<never> {
  return { ok: false, error: { code, message } };
}

export function evictOldestMapEntries<K, V>(
  entries: Map<K, V>,
  limit: number,
