...
| 注册玩家 ID 与展示名 | `playerId` 为 `acct_` 加随机串，在当前 `AccountStore` 内检查碰撞；`displayName` 在该 store 内忽略大小写拒绝重复；尚无独立公开 handle | `playerId` 保持内部稳定身份；新增可读 `@publicHandle`，不把展示名直接当协议键 |
...
| 终局再战 | 当前只有房主可触发 restart，随后回到 waiting，双方还要重新 ready | 先保留兼容行为，再以双方 rematch readiness 替代单方控制 |
...
4. 悔棋请求优先进入任务栏形成非阻塞响应：响应方同时看见接受/拒绝，请求方看见等待；当前明确 modal 只作为过渡兼容和小屏可访问性 fallback，不作为长期唯一入口。请求未解决前暂停落子和冲突动作。
...
1. `public` 房继续进入大厅，并可按策略允许房主 handle/account ID 解析；`unlisted` 房不进入大厅、不进入公共活跃房表，仍可通过规范房间码或邀请 URL 加入。
2. `unlisted` 只表示“不可公开发现”，不等于强访问控制。若产品承诺“只有收到邀请的人能进”，需要独立高熵 invite token、服务端授权校验、撤销/轮换和限流；不能仅依赖短房间码或前端隐藏。
3. `unlisted` 默认关闭 `allowJoinByHostHandle`，避免任何知道房主 handle 的人都能探测或进入朋友局；房主明确开启时才允许。
...
- 终局棋盘在双方达成前保持可复盘；新局开始后仍能从 record 打开上一局。
...
5. `IX-04A`：统一加入框、公开 handle 和显式房主别名索引，作为独立全栈提交。
6. `IX-04B`：unlisted 与邀请授权边界，作为独立全栈提交。
7. `IX-06`：终局历史与回放。
8. `IX-06A`：双方再战意愿与自动续局，作为独立全栈提交。
9. `IX-07`：精确大厅汇总，作为独立小型全栈提交。
10. `IX-08`、`IX-09`：分别重新研究、设计协议后实施。
