...
状态：实施中；`IX-00 + IX-01 + IX-02 + IX-03 + IX-04 + IX-04B + IX-04A` 已完成本地验证，下一步为 `IX-06A`
...
| `IX-06A` | 下一步 | 双方分别表达再战意愿；双方同意前保留终局棋盘，仅创建一次下一局 |

当前实施已新增服务端执行的 public/unlisted 可见性、当前 AccountStore 内唯一 public handle、显式房主目标索引和统一加入解析；仍没有高熵邀请授权 token、双方再战、activity summary、`matchConfig`、挑战或赛事协议。下一阶段独立实施 IX-06A；unlisted 继续只表示“不公开列出”，不冒充受邀请授权保护。
...
P0 的工作区、牌桌状态机与响应式布局以及 IX-04/04B/04A 大厅、可见性和统一加入闭环已经完成。下一步进入独立全栈 IX-06A；`matchConfig`、点名挑战和赛事属于后续能力，不能阻塞当前交互主线。
...
| 准备、落子、悔棋、认输、重开 | 已有服务端权限约束与状态驱动任务栏 | 双方再战仍待 IX-06A 替换单方 restart |
...
| Profile、棋谱、回放、SGF、排行榜 | 已有基础版，Profile 有独立路由 | Profile/排行榜已从大厅首屏下沉到按需展开区 |
