...
- 每条线包含 `id`、`name`、`minDifficulty`、`weight` 和相对中心坐标。
- 选择时先收集所有合法候选，不再命中第一条就直接返回。
...
- 命令：`npm run opening-book`。
- 文件：`tools/generate-opening-book.ts`。
- 输出格式：SGF，不自定义棋谱语法。
...
- 标准 26 开局来自公开连珠/五子棋资料：黑 1 天元、白 2 在中心 3x3、黑 3 在中心 5x5，去除对称后为 26 种。
- 26 开局分直接/间接两组，各 13 种。
- 棋谱交换格式采用 SGF；SGF 的 `GM[4]` 表示 Gomoku/Renju。
