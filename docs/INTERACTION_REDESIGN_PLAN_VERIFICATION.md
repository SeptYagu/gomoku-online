...
| E2-03 | 悔棋只能用当前阻塞 modal | 设计决策已完全同步 | 当前 modal 确实存在；BGA 研究同时给出“不使用阻塞式弹窗打断对局”（`:672-678`）。计划现已要求任务栏显示接受/拒绝和倒计时，并把“任何 fallback 不得完全遮住棋盘”写入三个目标视口的验收；这仍是待实现目标，不冒充当前 UI。 |
| E2-04 | 房主单方面 restart 足以表达再战 | 当前能力真实但产品逻辑偏弱 | PlayOK 研究直接支持终局留桌和连续再战（`:68-80`、`:112-154`）；横向结论的状态机另行提出 `rematch-ready`（`:1126-1137`）。当前源码只有 host restart；计划新增双方 readiness，属于本项目设计决策和后端新能力，并已解除对 IX-06 牌桌复盘 UI 的不必要依赖。 |
| E2-05 | 现有公开链接房足以等价朋友私密房 | 已否定并拆任务 | PaperGames 的公开产品路径强调朋友唯一链接（`:863-925`），但这只证明朋友入口值得做，不证明本项目当前链接房私密。计划继续把 unlisted/邀请授权作为独立全栈任务，并已解除它对 IX-04A public handle 的不必要依赖。 |
...
本轮把用户保存的九项竞品对比、四个待调整逻辑点和建议实施顺序重新映射到更新后的计划。核心产品方向一致，但发现两处不必要硬依赖和两处验收表达不足，现已同步更正：

| ID | 复核发现 | 判定与更正 |
| --- | --- | --- |
| K-01 | `IX-04B` 原先依赖 `IX-04A`，会让 unlisted 朋友链接等待账户 handle 迁移 | 已更正为只依赖 `IX-04`；handle/account-ID 发现策略成为 IX-04A 已落地时的可选分支。 |
| K-02 | `IX-06A` 原先依赖 `IX-06`，把双方再战排在完整牌桌复盘之后 | 已更正为依赖 `IX-02` 和当前已有 game record 保存；上一局先保证可从现有 Profile/record 查询，牌桌内直接复盘仍由 IX-06 后补。 |
| K-03 | 非阻塞悔棋没有明确禁止 fallback 完全遮住棋盘 | 已在 IX-02 动作、三个目标视口验收和最终验收中补充“接受/拒绝/倒计时与棋盘同时可辨认”。 |
| K-04 | scheng20 对照提出的朋友加入短路径没有点击约束 | 已在 IX-04 和最终验收中补充：邀请 URL 无中间确认；展开朋友流程后只输入并提交一次，不因组件拆分增加步骤。 |
| K-05 | 实施顺序把 IX-05、IX-06 放在朋友闭环和双方再战之前 | 已调整为 IX-04 后优先独立完成 IX-04B/IX-04A，再做 IX-06A，然后进入 IX-05/IX-06；IX-04A 与 IX-04B 不互相阻塞。 |

更新后的九项对齐判断没有反转：PlayOK、BGA、PaperGames 的主要差距已经成为明确 P1 任务；sen/gkoos 的独立 AI 和设置边界保持不变；minh100/scheng20 的短路径约束得到加强；FlyOrDie/Vint.ee 的比赛配置、挑战和赛事能力继续主动延期。这里描述的是计划一致性，不代表产品代码已经实现上述新任务。
...
- P0 完成后的默认顺序是大厅入口 → unlisted/统一加入别名（互不阻塞）→ 双方再战 → 安全切换/复盘 → 活跃度；这与本轮复核的竞品连续性优先级一致。
