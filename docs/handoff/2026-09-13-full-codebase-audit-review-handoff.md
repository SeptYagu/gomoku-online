...
- **通过项核验概况（极简）**：
  ...
  - 独立设计并运行了 11 项负向与极端边界用例（涵盖 225 子满盘、0ms 超时、非法出子、双玩家掉线与限流器 15,000 键压力测试），除揭示悔棋超时死代码（缺陷 1）外，其余用例均表现稳健符合预期。
...
- **缺陷修复落地记录**：
  1. **缺陷 1 (P3-1)**：已在 `src/server/domain/room-state-machine.ts:798` 移除不可达的死分支 `|| now >= undoRequest.expiresAt`，保留纯净的 `if (!accepted)` 状态判决，消除冗余死代码；
  2. **缺陷 2 (P3-2)**：已在 `README.md` 与 `deploy/openresty-gomoku.conf.example` 中补充单级与多级反向代理（如 CDN ➔ Nginx ➔ Node）部署契约及配置示例，明确指出反代必须通过 `$remote_addr` 或真实客户端请求头覆盖 `X-Forwarded-For`，防范 CDN 节点出口 IP 共享限流配额风险。
- **审查判定**：**全部缺陷已 100% 修复闭环（0×P0 / 0×P1 / 0×P2 / 0×P3 遗留），全仓代码与部署文档恢复可信基准状态**。
