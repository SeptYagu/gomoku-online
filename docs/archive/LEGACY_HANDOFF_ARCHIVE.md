...
## 2026-07-10 竞品复核增量与统一房间入口方案

### 用户目标

- 把“当前计划再次与竞品对比”的结论正式更新进计划文件。
- 评估注册玩家 ID 能否代替当前随机邀请码作为房间标识。
- 设计同一个加入输入框同时支持邀请码和房主 ID 的可行方案，并继续逐步记录工作。

### 步骤 1：核对账户 ID 与现有加入入口

1. 读取 `src/server/accounts.ts`、`src/server/room-store.ts` 和账户测试，确认注册账户的 `playerId` 由 `acct_` 加 8 字节随机 base64url 串生成；当前 `AccountStore` 会在自身 Map 中检查碰撞并持久化到 JSONL，但没有跨独立实例的全局唯一约束。
2. 确认注册 `displayName` 在当前 `AccountStore` 内按忽略大小写方式拒绝重复；它仍是展示名称，当前模型没有独立、可读、不可变的公开 handle。
3. 读取 `src/components/useFriendRoom.ts` 与 `GameShell.tsx`，确认加入输入现在只面向房间码：输入值被整体转成大写、长度限制为 8，提交后只发送 `room:join { roomCode }`。
4. 确认成功加入后，客户端 URL 和本地 room session 都保存服务端返回的规范 `RoomSnapshot.code`；这为“别名只负责解析、房间码继续作为主键”提供了现成边界。

### 步骤 2：核对房主生命周期并确定标识决策

1. 读取 `RoomStore.createRoom`、`leaveRoom`、`removePlayer` 和 `leaveParticipantRooms`，确认房主是可转移的 seat，而不是房间永久所有者；进行中房间离开时玩家会先标记断线，因此同一账户不能靠简单扫描房间得到永远唯一的目标。
2. 结论：现有 `acct_...` 技术上可作为注册身份的稳定备用查找值，但仍然随机难记，不能解决用户提出的传播问题，也不能替换 `roomCode`、URL、gameId 和历史记录中的规范房间键。
3. 方案：保留 `roomCode` 为规范主键；统一输入支持邀请 URL、房间码、`@publicHandle` 和原始 `acct_...`。其中公开 handle 是推荐给用户传播的可读标识，原始账户 ID 只作为兼容入口。
4. 不接受无前缀 display name：它会和房间码产生语法歧义，也会把可变展示信息变成协议键。游客房主继续只能通过房间码或邀请 URL 分享。
5. 解析与加入必须由服务端一次完成并返回规范房间码；房主别名最多指向一个当前可加入/可观战目标，房主转移、离房、房间关闭和多实例部署都必须有明确一致性规则。

### 步骤 3：更新交互计划与真实性核验

1. 更新 `docs/INTERACTION_REDESIGN_PLAN.md`，把“最多一个主动作”改为“一个主任务、1–2 个当前决策动作、总动作不超过 4”，并把悔棋响应改为优先进入非阻塞任务栏。
2. 新增 `IX-04A`：统一加入框、`@publicHandle`、原始账户 ID 兼容、服务端原子解析、规范房间码收敛和显式房主索引。
3. 新增 `IX-04B`：公开/unlisted 发现策略与真正邀请授权的边界，明确 unlisted 不自动等于强私密。
4. 新增 `IX-06A`：双方分别表达再战意愿，双方同意后服务端幂等创建下一局；现有 host restart 仅作为旧客户端兼容路径。
5. 更新任务优先级、允许文件边界、测试矩阵、实施顺序和最终验收标准；P0 第一批仍保持 `IX-00 + IX-01`，没有把新后端能力塞进首批重构。
6. 更新 `docs/INTERACTION_REDESIGN_PLAN_VERIFICATION.md`，新增竞品二次对照和账户/房间标识的 10 条增量核验，明确哪些是当前事实、哪些是设计决策、哪些必须实现后验证。

### 步骤 4：文档与当前行为验证

1. 首次运行文档结构检查时，PowerShell 在错误字符串的 `${doc}:line` 插值处发生解析错误；该命令没有修改文件。修正变量边界后重新执行成功。
2. Markdown 结构检查：计划和核验报告的代码围栏成对，连续表格的列分隔数量一致。
3. 完整 `file:line` 引用检查：识别到的 13 个完整路径引用均存在且行号未越界；简写 `:line` 仍由相邻完整文件上下文解释。
4. 过期措辞扫描：计划与核验报告中不再存在“最多 1 个主动作”“主动作唯一”或把双方 readiness 直接写成 PlayOK 已证实控件的措辞。
5. `git diff --check`：通过；只有 Windows 工作区 LF/CRLF 转换提示。
6. 定向回归 `npm test -- src/server/accounts.test.ts src/server/rooms.test.ts src/server/room-socket.test.ts`：3 个测试文件、54 个测试用例全部通过。
...
## 2026-07-10 竞品结论一致性完善

### 用户目标

- 根据上一轮“当前计划与既有竞品结论是否一致”的复核结果完善计划。
- 修正功能方向已经一致、但依赖和实施顺序仍与原结论不完全一致的部分。
- 继续把每个主要步骤记录到本 handoff。

### 步骤 1：确认需要完善的真实差异

1. 核对 `IX-04B` 当前硬依赖 `IX-04A`；unlisted 的核心能力只需要房间 visibility、列表过滤和服务端加入策略，并不需要先完成公开 handle 账户迁移，因此该依赖会无谓阻塞朋友链接闭环。
2. 核对 `IX-06A` 当前硬依赖 `IX-06` 且排在完整终局复盘之后；当前项目已经有 game record 保存和独立 Profile/record 回放基础，双方再战协议本身不必等待牌桌内历史 UI。
3. 核对 IX-02 已写“悔棋优先进入非阻塞任务栏”，但尚未把“任何目标视口都不能完全遮住棋盘”写成验收条件；modal fallback 的边界仍可能被误读。
4. 核对 IX-04 保留邀请 URL 自动加入，但没有明确限制组件拆分后的手动朋友加入步骤，尚未落实 scheng20 对照中的“不要因重构增加额外点击”约束。

### 步骤 2：完善计划依赖、验收和顺序

1. `IX-04B` 改为只依赖 `IX-04`；public/unlisted 服务端可见性可独立交付，`allowJoinByHostHandle` 仅在 `IX-04A` 已落地时作为可选字段和分支启用。
2. `IX-06A` 改为依赖 `IX-02` 与当前已有 game record 保存能力，不再依赖 `IX-06` 的牌桌内历史 UI；上一局先通过已有 Profile/record 路径保持可查询。
3. IX-02 明确任务栏同时显示接受/拒绝和倒计时，modal/drawer/无障碍 fallback 在 1440×900、1280×720、390×844 下都不得完全遮住棋盘。
4. IX-04 新增朋友短路径验收：邀请 URL 不经过额外确认页；手动流程展开后只输入目标并提交一次，不因组件拆分增加中间页。
5. 实施顺序调整为 `IX-04 -> IX-04B / IX-04A -> IX-06A -> IX-05 / IX-06 -> IX-07 -> IX-08/09`；IX-04A 与 IX-04B 都只依赖 IX-04，可交换或并行，默认朋友链接闭环优先。

### 步骤 3：同步真实性核验报告

1. 更新竞品二次核验表，记录悔棋遮挡边界、rematch 依赖和 unlisted 依赖均已同步更正。
2. 新增“与既有竞品结论的一致性复核”，逐条记录两处硬依赖、两处验收不足和实施顺序的处理结果。
3. 明确这些修改只完善计划一致性，当前产品代码仍未实现 public handle、unlisted、双方 rematch 或新的非阻塞悔棋 UI。

### 步骤 4：验证结果

1. Markdown 结构检查通过：计划与核验报告的代码围栏成对，连续表格列数一致。
2. 完整 `file:line` 引用检查通过：识别到的 13 个完整路径引用都存在且行号未越界。
3. 依赖断言通过：不存在 `IX-04B -> IX-04A` 或 `IX-06A -> IX-06` 的旧硬依赖，新依赖和“不完全遮住棋盘 / 只提交一次”验收措辞均存在。
4. 实施顺序自动断言通过：`IX-04 -> IX-04B -> IX-04A -> IX-06A -> IX-05 -> IX-06 -> IX-07`。
5. `git diff --check`：通过；只有 Windows 工作区 LF/CRLF 转换提示。
...
### 审查基线状态
- 测试与静态分析：`vitest run` 24 个测试文件 / 222 项用例全绿；`eslint .` 零错误零警告。
- 编译与类型检查：`tsc --noEmit` 0 错误（已彻底消除 8 条测试辅助基线错误，类型门禁全绿通过）。
...
#### P1: 消除 TypeScript 8 条基线编译错误（✅ 已修复，commit `1d763a0`）
...
2. **`TestSocket` 事件监听签名严格逆变冲突（6 处）**：
   - 位置：`src/server/room-socket.test.ts:24-30`
   - 修复：将 `TestSocket` 的监听器形参声明放宽为 `listener: TestSocketListener = (...args: never[]) => void`。`tsc --noEmit` 全仓零错误通过，且规避了 explicit-any lint 检查。

#### P2: 交互体验与状态副作用优化（✅ 已修复，commit `df2d5b3`）
1. **精简 `useFriendRoom` 中 auto-join effect 的依赖项（原 m12）**：
   - 位置：`src/components/useFriendRoom.ts`
   - 修复：提取 `currentRoomCode` 与 `hasRoom` 作为依赖项，去除响应式全量 `room` 对象依赖，对局内落子与状态变更不再重复调度 auto-join effect。
2. **修正大厅列表加载中错误借用按钮文案的问题（原 m13）**：
   - 位置：`src/components/online/OnlineLobbyView.tsx` 与 `src/i18n/dictionaries.ts`
   - 修复：在 `dictionary.room` 六语种中增加通用的 `loading` 文案（`Loading…` / `加载中…` 等），替换 Presence / 战绩 / 天梯列表的刷新按钮文本。
3. **消除个人战绩主页初载与手动刷新的重复逻辑（原 m17）**：
   - 位置：`src/components/profile/PlayerProfilePage.tsx`
   - 修复：抽取统一的 `loadProfile()` 异步请求函数，由初载 effect 和手动 `refreshProfile()` 共用，完全满足 `react-hooks/set-state-in-effect` 规范并消除重复代码。
4. **离房超时后的服务端/客户端短暂不一致（R9 取舍说明）**：
   - 位置：`src/components/useFriendRoom.ts:962-983`
   - 说明：维持既定取舍（重试可自愈）。

#### P3: AI 引擎优化与死代码清理（部分落地，commit `efc7f29`）
1. **修正成五胜局威胁被重复统计问题（原 m1，✅ 已修复）**：
   - 位置：`src/game/ai.ts` 与 `src/game/ai.test.ts`
   - 修复：`addWindowThreat` 遇到 `stones >= 5` 时直接 return，五连统计交由方向连续扫描独占，避免威胁分数虚高翻倍；增加单测验证。
2. **Web Worker 线程复用池（原 m9，待办）**：
   - 位置：`src/components/GameShell.tsx:437-454`
3. **开局库配置权重与难度差异化（原 m16，待办）**：
   - 位置：`src/game/opening-book.ts` 与 `src/game/ai.ts:548-599`
4. **清理 `rooms.ts` 中无调用的 `startGame` 死分支（原 m15，✅ 已修复）**：
   - 位置：`src/server/rooms.ts`
   - 修复：移除 `room.status === "ready"` 永假分支与死状态赋值代码，重构为纯状态验证（进行中返回快照，否则返回 `room-not-ready`）。

#### P4: 格式规范一致性与可访问性（✅ 已修复，commit `5748d12`）
1. **统一前后端 SGF 转义规范（原 m4 遗留）**：
   - 位置：`src/server/game-record-export.ts` 与 `src/game/game-record-sgf.ts`
   - 修复：从 `game-record-sgf.ts` 导出并复用 `escapeSgfValue` 与 `toSgfPoint`，统一 SGF FF[4] 控制字符过滤与换行转换规范。
2. **确认弹窗可访问性补全**：
   - 位置：`src/components/InteractionConfirmation.tsx`
   - 优化：增加全局 Escape 键监听及非提交状态下的取消调用，并增加 Tab / Shift+Tab 焦点循环控制。
...
### 42. 全量遗留缺陷修复与大厅实时汇总落地（2026-09-11）

针对前次审查中发现的所有大小问题（包含上一轮未做的遗留待办和新引入的隐患），已全量规划并完成修复：

#### 1. 修复新引入的 UI/A11y 焦点强夺与 Profile 状态残留隐患
- **弹窗焦点强夺修复**：`src/components/InteractionConfirmation.tsx` 将初次挂载自动聚焦 `cancelRef.current?.focus()` 解耦为独立 `useEffect(..., [])`，避免 `onCancel` 引用或组件重渲染时抢走正在编辑或浏览的元素焦点；同时在 `src/components/GameShell.tsx` 中使用 `useCallback` 稳定 `handleCancelPendingTransition` 回调引用。
- **Profile 页面状态残留修复**：在 `src/app/[locale]/profile/[playerId]/page.tsx` 中对 `<PlayerProfilePage key={decodedPlayerId} ... />` 传入 key，确保切换路由用户时自动完全重置组件树；同时在 `src/components/profile/PlayerProfilePage.tsx` 渲染期增加 `prevPlayerId !== playerId` 守卫，杜绝异地网络延迟造成的数据残留。

#### 2. 实现 AI Web Worker 线程复用池（P3-2 / 原 m9，✅ 已修复）
- **复用池实现**：新建 `src/game/ai-worker-pool.ts`，实现 `AiWorkerPool` 结构，统一管理 idle 闲置池与 busy 活跃 Worker。
- **调度策略**：AI 思考落子时优先复用 idle 实例，消除频繁 `new Worker()` 与垃圾回收抖动；只有在用户悔棋或重开取消思考时，精准 `terminateBusy()` 终止忙碌任务，其余正常对局保持线程常驻复用。
- **测试覆盖**：新增 `src/game/ai-worker-pool.test.ts`，验证复用获取、归还、忙碌终止和全量销毁。

#### 3. 实现开局库权重与四档难度差异化分级（P3-3 / 原 m16，✅ 已修复）
- **开局库分级**：`src/game/opening-book.ts` 将 26 条标准生成开局按风格与对策深度分为 Normal（8 条平稳型，权重 24~30）、Hard（8 条平衡对攻型，权重 20~24）、Expert（8 条复杂攻防型，权重 16~18）、Insane（2 条高变数奇兵型，权重 14）。
...
#### 4. 阶段 7 未完成项：落地 IX-07 精确大厅汇总（✅ 已完成）
...
- **Git 规则核验**：确认 `.gitignore` 已包含 `.codex/`、`.codex-remote-attachments/`、`tsconfig.tsbuildinfo` 及 `.workbuddy/*` 规则。

#### 6. 门禁验证结果
- `npx tsc --noEmit`：0 错误。
- `npm run lint`：0 错误 0 警告。
- `npm test`：26 套测试套件、228 项测试全数通过。
- `npm run build`：生产打包与 SSG 静态路由预渲染 100% 成功。
