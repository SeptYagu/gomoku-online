...
- 阶段 3 已完成大厅房间列表、Join/Watch、观战席、公共/房间聊天、随机匹配和用户 Presence 第一版。
...
- 继续公开测试：邀请试玩，重点记录移动端误触、guest token 恢复、多标签连接和在线服务稳定性问题。
- 补多语言 metadata、canonical/alternate 和 sitemap 基础 SEO。
- 为在线房间继续补 Redis Adapter、共享限流、正式数据库和多实例状态。
- 继续账号完整化、排行榜增量事件，以及 opening analysis 候选接入 arena/运行时开局库。
- 继续细化移动端好友房手感和小屏视觉回归。

## 对弈规则（Freestyle Gomoku）

本项目采用**自由五子棋规则（Freestyle Gomoku）**：

- **棋盘规格**：标准 15x15 路棋盘（`BOARD_SIZE = 15`）。
- **胜负判定**：黑白双方轮流落子，任意一方率先在横、竖、正斜、反斜任意单一方向形成连续 5 颗或 5 颗以上本方棋子（`>=5` 连珠，包含长连）即判定获胜（源码判决逻辑见 `src/game/board.ts:140` 与 `src/game/ai.ts:1633`）。
- **无禁手限制**：黑棋无三三、四四、长连等禁手，先手长连亦直接判胜，规则完全对称公平，体验轻量直观。
- **和棋规则**：棋盘 225 格全部填满且无任何一方连珠达 5 子时判定为平局。
