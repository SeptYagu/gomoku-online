...
- 代码评审报告在 docs/code-review-2026-09-10.md 与 docs/code-review-2026-09-10-followup.md（两份都含滚动修复进展表）。已完成：M1+M2（7be68a2）、M3（d69d0bd）、M4/M5/m6/m7（f30e273）、复审 R1+R2（e14ec7e）、R3/R4/R5+R7（5056016，Codex 修的，已做变异复核 87b457d）、M6+M7+R8+m3/m4/m5/m8/m10/m11/m14（2a2c2b6）。**只剩 R9 未修**（超时后「服务端已离开、客户端仍认为在房间」，R4 忽略迟到 ack 的既定取舍，重试自愈）与 R6 部分保留（boot mode 已实例化闭包，四个 `boot*Cache` 仍模块级）；其余 Minor/Nit：m1/m9/m12/m13/m15/m16/m17 与架构拆分。
