...
- 代码评审报告在 docs/code-review-2026-09-10.md 与 docs/code-review-2026-09-10-followup.md（两份都含滚动修复进展表）。已完成：M1+M2（7be68a2）、M3（d69d0bd）、M4/M5/m6/m7（f30e273）、复审 R1+R2（e14ec7e）。未完成：R3 超时/连接错误文案缺多语种（根因是 hook 拿不到 dictionary，需先把 locale 注入 hook）、R4 超时后迟到 ack 仍清理房间状态与「告知调用方 left=false」冲突、R5 leaveRoom 8s 计时器无卸载清理、R7 leaveRoom 超时语义与 boot snapshot 缺测试；以及 M6 持久化无界增长、M7 XFF 可伪造绕过限流（**需先确认线上是否部署在可信反向代理之后，否则改默认值会误伤现有限流**）、m10/m11 及其余 Minor/Nit。
